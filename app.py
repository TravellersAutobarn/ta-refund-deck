...
from generate_branch_praise_deck import generate_region as generate_branch_praise_region
...
def generate_us_info_trends():
    ...
    If us_deck exists, this route extracts only us_deck.
    """
    data = request.get_json()
    ...
    deck_data = data.get("us_deck", data)

    if not isinstance(deck_data, dict):
        return jsonify({"error": "us_deck must be a JSON object"}), 400

    deck_data["date_label"] = date_label
...
def generate_aunz_info_trends():
    ...
    If aunz_deck exists, this route extracts only aunz_deck.
    """
    data = request.get_json()
    ...
    date_label = data.get("date_label", "Previous Month")
    deck_data = data.get("aunz_deck", data)

    if not isinstance(deck_data, dict):
        return jsonify({"error": "aunz_deck must be a JSON object"}), 400
...
def _run_wwyd_region(region, filename):
    ...
    Accepts raw JSON output and tolerates markdown fences.
    """
    import traceback
...
def _run_negotiation_region(region, filename):
    ...
    as a PowerPoint file. Accepts raw JSON output and tolerates markdown fences.
    """
    import traceback
    import json as _json
    import re as _re
...
def _run_branch_praise_region(region, filename):
    """
    Build one region's Branch Praise deck and return it as a PowerPoint file.

    Accepts either:
    1. A region-only payload
    2. A combined payload with us_deck and aunz_deck keys
    """
    import traceback
    ...
    try:
        ...
        if region == "america":
            deck_data = payload.get("us_deck", payload)
        elif region == "aunz":
            deck_data = payload.get("aunz_deck", payload)
        else:
            deck_data = payload

        if not isinstance(deck_data, dict):
            return jsonify({"error": "Deck payload must be a JSON object"}), 400

        outdir = tempfile.mkdtemp(prefix="branch_praise_")
        path = generate_branch_praise_region(deck_data, outdir, region)

        if not path:
            return jsonify({
                "error": "No branch praise deck could be generated for region '%s'" % region
            }), 400
...
@app.route("/generate-branch-praise-deck/aunz", methods=["POST"])
def branch_praise_deck_aunz():
    return _run_branch_praise_region("aunz", "branch_praise_aunz.pptx")


@app.route("/generate-branch-praise-deck/america", methods=["POST"])
def branch_praise_deck_america():
    return _run_branch_praise_region("america", "branch_praise_america.pptx")
